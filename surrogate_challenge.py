...
def fit_surrogate(train_df, feature_names, teacher):
    ...
    # Fix: fit scaler on the full training set, not just the sample.
    ...
    # Fix: use the teacher's predictions as labels, not the ground-truth target.
    # The surrogate must learn to mimic the teacher's output, not the raw y.
    # Teacher was trained on raw features, so pass raw (un-scaled) features here.
    ...
    # Fix: refit the best model on the full training set with teacher labels
    # so the surrogate sees the complete input distribution.
    best = XGBRegressor(
        objective="reg:squarederror",
        min_child_weight=12,
        random_state=RANDOM_STATE,
    ...
    best.fit(
        ...
        teacher.predict(train_df[feature_names].values),
    )
